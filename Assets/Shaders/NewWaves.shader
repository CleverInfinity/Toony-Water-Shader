Shader "Custom/NewWaves"
{
	Properties
	{
		...
		_Shininess("Shininess", Range(1.0, 50.0)) = 2.0

		_WaveHeight("Wave Height", Range(0.0, 1.1)) = 0.25
        _WaveSpeed("Wave Speed", Range(0.0, 10.0)) = 1.0
		_WaveSteepness("Wave Steepness", Range(0.0, 0.5)) = 0.2
		_WaveLength("Wave Length", Range(1.0, 4.0)) = 2.5
		...
		_TessellationFadeStartDistance("Tessellation Fade Start Distance", float) = 25
		_TessellationFadeEndDistance("Tessellation Fade End Distance", float) = 50
	...
	SubShader
	{
		...
		Pass{
			...
			tessFactors patchConstantFunc(InputPatch<tessControlPoint, 3> patch)
			{
				...
				//get camera position
				float camPos = _WorldSpaceCameraPos;
			...
			float3 SumGerstnerWaves(float3 positionWS)
			{
				...
				for (int i = 0; i < (int)_WaveNumber; i++)
				{
					...
					float angle = hash(fi * 1.0) * 6.28318; // direction
					float2 dir = normalize(float2(cos(angle), sin(angle)));
					float lenVar = lerp(0.8, 1.2, hash(fi * 2.0 + 7.0));  
					float speedVar = lerp(0.8, 1.2, hash(fi * 3.0 + 13.0));
			...
			t2f domain(tessFactors factors, OutputPatch<tessControlPoint, 3> patch, float3 barycentricCoordinates : SV_domainLocation)
			{
				...
				output.waveHeight = waveDisplacement.y;
			...
			{
				float2 random = float2(dot(value, float2(127.1, 311.7)),dot(value, float2(269.5, 183.3)));

				return frac(sin(random) * 43758.5453);
			}
			
			float3 randomfloat3(float3 value)
			{
				float3 random = float3(dot(value,float3(12.989, 78.233, 37.719)),dot(value,float3(39.346, 11.135, 83.155)),dot(value,float3(73.156, 52.235, 09.151)));
				return frac(sin(random) * 43758.5453);
			}
			...
			float3 voronoiNoise(float3 value){
				...
				for(int x = -1; x<=1; x++)
				{
					for(int y = -1 ; y <= 1 ; y++ )
					{
						for(int z = -1; z<= 1; z++){
							...
							float3 cellPosition = cell + float3(randomfloat3(cell));
				...
				for(int x1 = -1; x1<=1; x1++)
				{
					for(int y1 = -1 ; y1 <= 1 ; y1++ )
					{
						for(int z1 = -1; z1<= 1; z1++){
							...
							float3 cellPosition = cell + randomfloat3(cell);
							float3 toCell = cellPosition - value;
				...
				float3 cellRandom = randomfloat3(closestCell);
			...
			float4 fragment(t2f input) : SV_TARGET
			{
				...
				half3 finalColor = lerp(_BaseColor.rgb, _DeepColor.rgb, depthFade);
				...
				float crestMask = saturate((abs(input.waveHeight)) * 5.0);
				
				finalColor = lerp(finalColor, float3(1,1,1), border * crestMask * 0.7);
